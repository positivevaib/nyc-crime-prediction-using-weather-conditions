...
object ProfileData {
  def profileData(sc: SparkContext, mode: String, rawPath: String, cleanPath: String): Unit = {
    ...
    // Crime Data - count number of each crime type in Raw and Clean Data
    if(mode == "Crime"){
    	...
    	for (i <- 0 to 70){
		val CountRawData = rawCrimeData.filter(line => (line(8) == CrimeTypeArray(i.toInt))).count
        	val CountCleanData = cleanData.filter(line => line(4) == i.toString).count
		println(i + " - " + CrimeTypeArray(i.toInt) + ": " + CountRawData + " | "+CountCleanData)
    	}   
     }
  }
